...
class BaseX2HAttLayer(nn.Module):
    ...
    def forward(self, h, r_feat, edge_feat, edge_index, fix_mask, mask_ligand, e_w=None):
        N = h.size(0)
        src, dst = edge_index
        hi, hj = h[dst], h[src]
        ...
        fix_mask_onehot = fix_mask == 1.
        fix_mask_index = torch.where(fix_mask_onehot)[0]
        mask_ligand_edit = mask_ligand.clone()
        mask_ligand_edit[torch.where(mask_ligand_edit)[0][fix_mask_index]] = False
        #x = x + delta_x * mask_ligand_edit[:, None]
        #output = h + output
        output = h + output * mask_ligand_edit[:, None]
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, mask_ligand, fix_mask, e_w=None, fix_x=False):
        src, dst = edge_index
        ...
        for i in range(self.num_x2h):
            ...
            h_out = self.x2h_layers[i](h_in, dist_feat, edge_feat, edge_index, fix_mask=fix_mask, mask_ligand=mask_ligand, e_w=e_w)
            # edge_feat : edgy type (4 length one-hot vector)
            h_in = h_out
        x2h_out = h_in
        ...
        for i in range(self.num_h2x):
            ...
            if not fix_x:
                ...
                mask_ligand_edit = mask_ligand.clone()
                mask_ligand_edit[torch.where(mask_ligand_edit)[0][fix_mask_index]] = False
                x = x + delta_x * mask_ligand_edit[:, None]  # only ligand positions will be updated
            rel_x = x[dst] - x[src]
            dist = torch.norm(rel_x, p=2, dim=-1, keepdim=True)
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    @staticmethod
    def _build_edge_type(edge_index, mask_ligand, fix_mask):
        src, dst = edge_index
        ...
        mask_ligand_edit = mask_ligand.clone()
        mask_ligand_edit[torch.where(mask_ligand_edit)[0][fix_mask_index]] = False
        fix_src = mask_ligand_edit[src] == 1
        fix_dst = mask_ligand_edit[dst] == 1        
        
        edge_type[fix_src & ~n_dst] = 4
        edge_type[~n_src & fix_dst] = 5
        edge_type[fix_src & n_dst] = 6
        edge_type[n_src & fix_dst] = 7
        edge_type[fix_src & fix_dst] = 8
        edge_type = F.one_hot(edge_type, num_classes=9)
        return edge_type

    def forward(self, h, x, mask_ligand, batch, fix_mask, return_all=False, fix_x=False):
        ...
        for b_idx in range(self.num_blocks):
            ...
            # edge type (dim: 4)
            edge_type = self._build_edge_type(edge_index, mask_ligand, fix_mask)
            if self.ew_net_type == 'global':
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, x = layer(h, x, edge_type, edge_index, mask_ligand, fix_mask=fix_mask, e_w=e_w, fix_x=fix_x)
            all_x.append(x)
            all_h.append(h)
