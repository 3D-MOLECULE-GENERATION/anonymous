...
def center_pos(protein_pos, ligand_pos, batch_protein, batch_ligand, mode='protein'):
    if mode == 'none':
        ...
        # ligand_pos: batch.ligand
        # batch_ligand: number of prior
    else:
...
class PharDiff(nn.Module):
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand, fix_mask,
        ...
        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, fix_mask=fix_mask, return_all=return_all, fix_x=fix_x)
    ...
    def q_v_pred(self, log_v0, t, batch):
        # compute q(vt | v0)
        log_cumprod_alpha_t = extract(self.log_alphas_cumprod_v, t, batch)  ## >> alpha_bar_t
        log_1_min_cumprod_alpha = extract(self.log_one_minus_alphas_cumprod_v, t, batch)  ## >> 1-alpha_bar_t
    ...
    def get_diffusion_loss(
        ...
        a_pos_mask = torch.tensor([]).to("cuda")
        for b in range(batch_protein.max().item() + 1):
            b_a_pos = a_pos[batch.ligand_element_batch==b]
            #print(batch.fix_node[batch.fix_node_batch==b])
            #print(b_a_pos)
            #print("===========================")
            b_a_pos[batch.fix_node[batch.fix_node_batch==b].type('torch.LongTensor')] = 1.
            a_pos_mask = torch.cat([a_pos_mask, b_a_pos], 0)  
        # a_pos_mask = torch.tensor([]).to("cuda")
        # for b in range(batch_protein.max().item() + 1):
        #     b_a_pos = a_pos[ligand_element_batch==b]
        #     b_a_pos[fix_node[fix_node_batch==b]] = 1.
        #     a_pos_mask = torch.cat([a_pos_mask, b_a_pos], 0)    
                
        ligand_pos_perturbed = a_pos_mask.sqrt() * ligand_pos + (1.0 - a_pos_mask).sqrt() * pos_noise  # pos_noise * std
        
        # ligand_v : ligand one-hot atom types
        # Vt = a * V0 + (1-a) / K
        ...
        #loss_pos = ((pred[(a_pos_mask != 1.)[:,0]] - target[(a_pos_mask != 1.)[:,0]])**2).sum()
        loss_pos = scatter_mean(((pred[(a_pos_mask != 1.)[:,0]] - target[(a_pos_mask != 1.)[:,0]]) ** 2).sum(-1), batch_ligand[(a_pos_mask != 1)[:,0]], dim=0)
        loss_pos = torch.mean(loss_pos)
        ...
        nf = target[(a_pos_mask != 1.)[:,0]]
        f = target[(a_pos_mask == 1.)[:,0]]
        f_repeat = f.unsqueeze(1).repeat(1, nf.size(0), 1)
        nf_repeat = nf.unsqueeze(0).repeat(f.size(0), 1, 1)
        target_distance = torch.sqrt((torch.pow(f_repeat - nf_repeat, 2).sum(-1)))
        target_dirvec = (f_repeat - nf_repeat)/(f_repeat - nf_repeat).norm(dim=(2)).unsqueeze(-1)


        nf = pred[(a_pos_mask != 1.)[:,0]]
        f = target[(a_pos_mask == 1.)[:,0]]
        f_repeat = f.unsqueeze(1).repeat(1, nf.size(0), 1)
        nf_repeat = nf.unsqueeze(0).repeat(f.size(0), 1, 1)
        pred_distance = torch.sqrt((torch.pow(f_repeat - nf_repeat, 2).sum(-1)))
        pred_dirvec = (f_repeat - nf_repeat)/(f_repeat - nf_repeat).norm(dim=(2)).unsqueeze(-1)

        loss_dir = scatter_mean(((pred_dirvec - target_dirvec)**2).sum(-1).sum(-1), batch_ligand[(a_pos_mask == 1)[:,0]], dim=0)

        loss_dis = scatter_mean(((pred_distance - target_distance)**2).sum(-1), batch_ligand[(a_pos_mask == 1)[:,0]], dim=0)
        
        loss_dis = torch.mean(loss_dis) * 0.1 # 0.01
        loss_dir = torch.mean(loss_dir)
        
        loss_dis = (loss_dis + loss_dir)
        #loss_pos = loss_pos + loss_dis
        ...
        # fix_node exception
        log_v_model_prob = self.q_v_posterior(log_ligand_v_recon[(a_pos_mask != 1.)[:,0]], 
                                              log_ligand_vt[(a_pos_mask != 1.)[:,0]], 
                                              time_step, batch_ligand[(a_pos_mask != 1.)[:,0]])
        log_v_true_prob = self.q_v_posterior(log_ligand_v0[(a_pos_mask != 1.)[:,0]], 
                                             log_ligand_vt[(a_pos_mask != 1.)[:,0]], 
                                             time_step, batch_ligand[(a_pos_mask != 1.)[:,0]])
        
        kl_v_ = self.compute_v_Lt(log_v_model_prob=log_v_model_prob, 
                                 log_v0=log_ligand_v0[(a_pos_mask != 1.)[:,0]],
                                 log_v_true_prob=log_v_true_prob, 
                                 t=time_step, batch=batch_ligand[(a_pos_mask != 1.)[:,0]])
        
        
        loss_v = torch.mean(kl_v)
        loss_v_ = torch.mean(kl_v_)
        
        #loss_v = (loss_v + loss_v_) * 0.5
        loss_v = loss_v_ 
...
########################
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand, batch,
        ...
        _, fix_ligand_pos, _ = center_pos(
            batch.protein_pos, batch.ligand_pos, batch_protein, batch.ligand_element_batch, mode='protein')
                       
        a = torch.rand(num_graphs)
        a_pos = a[batch_ligand].unsqueeze(-1).to("cuda")
        
        a_pos_mask = torch.tensor([]).to("cuda")
        for b in range(batch_protein.max().item() + 1):
            b_a_pos = a_pos[batch_ligand==b]
            #b_a_pos[batch.fix_node[batch.fix_node_batch==b]] = 1.
            b_a_pos[batch.fix_node[batch.fix_node_batch==b].type('torch.LongTensor')] = 1.
            a_pos_mask = torch.cat([a_pos_mask, b_a_pos], 0)

            
        a_pos = a[batch.ligand_element_batch].unsqueeze(-1).to("cuda")
        a_pos_mask_fix = torch.tensor([]).to("cuda")
        for b in range(batch_protein.max().item() + 1):
            b_a_pos = a_pos[batch.ligand_element_batch==b]
            #b_a_pos[batch.fix_node[batch.fix_node_batch==b]] = 1.
            b_a_pos[batch.fix_node[batch.fix_node_batch==b].type('torch.LongTensor')] = 1.
            a_pos_mask_fix = torch.cat([a_pos_mask_fix, b_a_pos], 0)
            

        init_ligand_pos[a_pos_mask[:,0] == 1] = fix_ligand_pos[a_pos_mask_fix[:,0] == 1] 
        #init_ligand_pos[torch.where(a_pos_mask[:,0] == 1)[0]] = fix_ligand_pos[torch.where(a_pos_mask[:,0] == 1)[0]]
        
        ## fix
        init_ligand_v[a_pos_mask[:,0] == 1] = batch.ligand_atom_feature_full[a_pos_mask_fix[:,0] == 1] 
        #init_ligand_v[torch.where(a_pos_mask[:,0] == 1)[0]] = batch.ligand_atom_feature_full[torch.where(a_pos_mask[:,0] == 1)[0]]
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            ligand_pos[a_pos_mask[:,0] == 1] = fix_ligand_pos[a_pos_mask_fix[:,0] == 1]
            ...
            if not pos_only:
                ...
                ## fix 
                log_model_prob[a_pos_mask[:,0] == 1] = index_to_log_onehot(batch.ligand_atom_feature_full, self.num_classes)[a_pos_mask_fix[:,0] == 1]
                #log_model_prob[a_pos_mask[:,0] == 1] = init_ligand_v[a_pos_mask[:,0] == 1]
