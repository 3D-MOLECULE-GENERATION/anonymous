...
def rand_rotate(dir, ref, pos, alpha=None):
    ...
    rotated_pos = torch.mm(T.type(torch.float64), pos.type(torch.float64))[:3]
...
def enum_attach(ctr_mol, nei_mol):
    ...
    if nei_mol.GetNumBonds() == 1:  # neighbor is a bond
        ...
        # intersection is two connected bonds
        rings = [set(x) for x in Chem.GetSymmSSSR(ctr_mol)]
        intersection = []
        if len(rings)>=2:
            for index1 in range(len(rings)-1):
                for index2 in range(index1+1, len(rings)):
                    intersection.extend(list(rings[index1] & rings[index2]))
            if len(intersection)>0:
                for inter in intersection:
                    a1 = ctr_mol.GetAtomWithIdx(inter)
                    a1_nei = [nei for nei in a1.GetNeighbors() if nei.GetIdx() not in intersection]
                    if len(a1_nei)!= 2:
                        continue
                    a1b1 = ctr_mol.GetBondBetweenAtoms(inter, a1_nei[0].GetIdx())
                    a1b2 = ctr_mol.GetBondBetweenAtoms(inter, a1_nei[1].GetIdx())
                    for a2 in nei_mol.GetAtoms():
                        a2_nei = [nei for nei in a2.GetNeighbors()]
                        if len(a2_nei) != 2:
                            continue
                        a2b1 = nei_mol.GetBondBetweenAtoms(a2.GetIdx(), a2_nei[0].GetIdx())
                        a2b2 = nei_mol.GetBondBetweenAtoms(a2.GetIdx(), a2_nei[1].GetIdx())
                        if atom_equal(a1, a2) and a1b1.GetBondType() == a2b1.GetBondType() and a1b2.GetBondType() == a2b2.GetBondType()\
                                and atom_equal(a1_nei[0], a2_nei[0]) and atom_equal(a1_nei[1], a2_nei[1]):
                            amap = {a2.GetIdx(): a1.GetIdx(),
                                    a2_nei[0].GetIdx(): a1_nei[0].GetIdx(), a2_nei[1].GetIdx(): a1_nei[1].GetIdx()}
                            att_confs.append(amap)

                        if atom_equal(a1, a2) and a1b2.GetBondType() == a2b1.GetBondType() and a1b2.GetBondType() == a2b1.GetBondType() \
                                and atom_equal(a1_nei[0], a2_nei[1]) and atom_equal(a1_nei[1], a2_nei[0]):
                            amap = {a2.GetIdx(): a1.GetIdx(),
                                    a2_nei[1].GetIdx(): a1_nei[0].GetIdx(), a2_nei[0].GetIdx(): a1_nei[1].GetIdx()}
                            att_confs.append(amap)
